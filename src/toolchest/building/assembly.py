...
# i.e. under 25 ms. Every IMoVE static pose exceeds it, which is the point -- a subject
# standing still gives two noise traces and no peak to find.
...
def align_world_to_imu(plate: 'PlateTrial') -> 'PlateTrial':
    ...
    #
    #    This became load-bearing when alignment stopped trimming. The padded stretches
    #    hold a constant pose, so their synthetic gyro is identically zero while the real
    #    sensor is still moving. Fitting over them would drag the rotation toward
    #    whatever explains a few hundred seconds of "the segment did not rotate", and on
    #    Subject01 that is 430 s against 603 s of real overlap — the offset would be
    #    decided by the padding rather than by the data.
...
def _to_common_grid(imu_trace: IMUTrace, world_trace: WorldTrace, target_rate: float,
    ...
    t = 0 sits at the first overlapping sample, the same instant the old code re-zeroed to,
    so a filter started at t = 0 sees an identical first sample and identical burn-in.
    Inertial data before the overlap lives at negative time.
    """
    ...
    world_trace = _world_on_timestamps(world_trace, world_times, target_rate)

    observed = np.flatnonzero(world_trace.valid)
    if len(observed) == 0:
        raise ValueError("IMU and mocap records do not overlap at all.")
    origin = imu_trace.timestamps[observed[0]]
