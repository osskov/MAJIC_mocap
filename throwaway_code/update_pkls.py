...
def update_specific_method(method_to_update: str, 
    ...
    # 3. Loop, regenerate, and collect new data for the method
    newly_generated_dfs = []
    for trial_type in trial_types:
        for subject_id in subjects:
            print(f"--- Reprocessing {subject_id} - {trial_type} for method '{method_to_update}' ---")
            
            # We need both 'Marker' (as reference) and the method to update
            methods_to_process = ['Marker', method_to_update]
            
            try:
                # Use the existing loading function but only for the two relevant methods
                ...
                if not subject_df.empty:
                    ...
                    if not method_specific_df.empty:
                        ...
                        print(f"Successfully regenerated {len(method_specific_df)} rows for '{method_to_update}'.")
                    else:
                        print(f"Warning: Regeneration for {subject_id} - {trial_type} yielded no data for '{method_to_update}'.")
                else:
                    print(f"Warning: No data returned when reprocessing {subject_id} - {trial_type}.")

            except Exception as e:
                print(f"An error occurred while reprocessing {subject_id}: {e}. Skipping.")
                continue
        ...
        for trial_type in TRIAL_TYPES:
            for subject_id in SUBJECT_IDS:
                ...
                try:
                    ...
                    # We don't save individual pickles in this simplified flow
                    # if not subject_df.empty:
